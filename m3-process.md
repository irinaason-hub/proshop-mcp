...
На финальном этапе перед сдачей еще ИИ обнаружил, что в инструментах нет примеров. Добавила в конце.
...
## Этап 3. Подключение к хосту и прогон сценария

**Задача:** Подключить готовый MCP-сервер к Claude Code как хосту, убедиться, что список инструментов и их схемы отдаются корректно, снять скриншот из Inspector, и прогнать оба обязательных сценария — цепочку из тестового промпта и запрос на запрещённый переход, зафиксировав в отчёте реальные цепочки вызовов.

**План:** Разбила на две части, потому что они проверяют разное. 

Сначала инфраструктурная часть — установить и настроить сам Claude Code (до этого момента он не был установлен), зарегистрировать сервер на project scope через `claude mcp add`, убедиться через `/mcp`, что статус connected, и только после этого снять скриншот из MCP Inspector линии V1 — отдельно от хоста, чтобы скриншот не зависел от того, поднимется ли сам Claude Code. 

Вторая часть — собственно прогон: сначала тестовый сценарий (агент сам строит цепочку read → set_payment_method → change_order_status → read), затем отдельным промптом — запрос на прямой запрещённый переход, чтобы проверить не падение сервера, а понятность текста отказа.

**Промпты:**

Промпт 1 (Разрешенный сценарий)
```
...
Промпт 2 (Цепочка вызовов разрешенного сценария)
```
